...
const PROFILE_VALUES = {
  standard: { privacyLevel: 'standard', letterbox: false, blockTrackers: true, blockAds: true, blockSocialTrackers: true, heuristicTrackingProtection: false, blockFingerprintingScripts: true, blockThirdPartyCookies: true, stripTrackingParams: true, stripCrossSiteReferrers: true, disableServiceWorkers: false },
  strict: { privacyLevel: 'strict', letterbox: true, blockTrackers: true, blockAds: true, blockSocialTrackers: true, heuristicTrackingProtection: true, blockFingerprintingScripts: true, blockThirdPartyCookies: true, stripTrackingParams: true, stripCrossSiteReferrers: true, disableServiceWorkers: false },
  maximum: { privacyLevel: 'maximum', letterbox: true, blockTrackers: true, blockAds: true, blockSocialTrackers: true, heuristicTrackingProtection: true, blockFingerprintingScripts: true, blockThirdPartyCookies: true, stripTrackingParams: true, stripCrossSiteReferrers: true, disableServiceWorkers: true }
};
...
function renderSecuritySuite() {
  ...
  const counts = suite?.summary || { pass: 0, fail: 0, 'not-tested': 0, total: 0 };
  summary.textContent = suite ? `${counts.pass || 0} pass · ${counts.fail || 0} fail · ${counts['not-tested'] || 0} not tested` : 'No verification run yet';
  ...
  checks.forEach((check) => {
    ...
    badge.textContent = check.status === 'pass' ? 'PASS' : (check.status === 'fail' ? 'FAIL' : 'NOT TESTED');
...
async function runSecuritySuite() {
  ...
  try {
    ...
    showToast({
      message: failed ? `Security Suite completed with ${failed} failed check${failed === 1 ? '' : 's'}. Review the evidence below.` : 'Security Suite verification completed with no failed checks.',
...
const draftControlIds = [
  'blockTrackers','blockAds','blockSocialTrackers','heuristicTrackingProtection','siteIntelligence','blockFingerprintingScripts','blockThirdPartyCookies','stripTrackingParams','unwrapTrackingLinks','etagProtection','publicCdnIsolation','stripCrossSiteReferrers','letterboxToggle',
  ...
  'proxyMode','proxyServer','proxyBypassLocal','proxyFailClosed','homePage','searchEngine','customSearchTemplate','customFilterRules','themeSelect','densitySelect','accentSelect',
