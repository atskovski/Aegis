...
const { configurePrivacySession, makeTabStats, freshSeed, isRiskyDownload, defaultDownloadPath, safeOrigin } = require('./core/privacy');
...
const { fetchPublicIp, testSessionIsolation, makeCheck, summarizeChecks } = require('./core/security-suite');
const { buildCosmeticCss } = require('./core/cosmetic');
...
async function runSecuritySuite() {
  ...
  if (tab) {
    ...
    checks.push(makeCheck('renderer-isolation', 'Renderer isolation', rendererOk ? 'pass' : 'fail',
      rendererOk
        ? 'Active web renderer was created sandboxed with context isolation, Node disabled, webSecurity enabled, and DevTools disabled.'
    ...
    checks.push(makeCheck('permission-firewall', 'Permission firewall', tab.permissionFirewallReady ? 'pass' : 'fail',
      tab.permissionFirewallReady ? 'Both permission-request and permission-check handlers are installed for this private tab session.' : 'Permission handlers are not confirmed for the active tab.', 'runtime-policy'));
    const fpNeeded = settings.privacyLevel !== 'standard';
    checks.push(makeCheck('fingerprint-defense', 'Fingerprint defense', (!fpNeeded || tab.fingerprintReady) ? 'pass' : 'fail',
      fpNeeded ? (tab.fingerprintReady ? 'Anti-fingerprint preload and CDP normalization initialized successfully.' : 'Strict/Maximum fingerprint defenses did not report ready.') : 'Standard mode intentionally uses reduced fingerprint normalization.', 'runtime'));
    ...
    checks.push(makeCheck('renderer-isolation', 'Renderer isolation', 'not-tested', 'No active web tab.', 'runtime'));
    checks.push(makeCheck('permission-firewall', 'Permission firewall', 'not-tested', 'No active web tab.', 'runtime'));
    checks.push(makeCheck('fingerprint-defense', 'Fingerprint defense', 'not-tested', 'No active web tab.', 'runtime'));
    checks.push(makeCheck('https-first', 'HTTPS-first navigation', 'not-tested', 'No active web tab.', 'runtime'));
  }

  checks.push(makeCheck('webrtc-leak-guard', 'WebRTC leak guard', 'pass',
  ...
  checks.push(makeCheck('tracker-blocking', 'Tracker blocking', settings.blockTrackers ? 'pass' : 'fail',
    settings.blockTrackers ? 'Native main-process request blocking is enabled.' : 'Tracker blocking is disabled in Settings.', 'runtime-policy'));
  ...
  try {
    ...
    checks.push(makeCheck('public-ip', 'Routed public IP', publicIp.status,
      publicIp.ok ? `Observed ${publicIp.ip} through the configured Chromium route (${publicIp.provider}).` : `Public IP lookup failed: ${publicIp.error}`, 'external-proof'));
  } catch (err) {
    checks.push(makeCheck('dns-https', 'DNS + HTTPS reachability', 'fail', `Routing/connectivity test failed: ${err.message}`, 'behavioral-test'));
    checks.push(makeCheck('public-ip', 'Routed public IP', 'not-tested', 'Public IP could not be checked because the disposable routed session failed.', 'external-proof'));
  } finally {
    try { await ses.clearData(); } catch {}
    try { await ses.clearCache(); } catch {}
    try { await ses.closeAllConnections(); } catch {}
...
async function installFingerprintDefenses(tab) {
  ...
  try {
    ...
    await withTimeout(dbg.sendCommand('Page.addScriptToEvaluateOnNewDocument', {
      source: buildAntiFingerprintScript({ seed: `${identitySeed}:${tab.seed}`, chromiumMajor: chromiumMajor(), profile: settings.privacyLevel, disableServiceWorkers: settings.disableServiceWorkers })
...
function wireTabView(tab, view) {
  ...
  view.webContents.on('did-finish-load', async () => {
    ...
    if (tab.shieldsEnabled && settings.blockAds && !tab.compatibilityMode) {
      try {
        const aggressive = settings.privacyLevel === 'maximum';
        await view.webContents.insertCSS(buildCosmeticCss({ aggressive }), { cssOrigin: 'author' });
      } catch (err) { console.warn('Cosmetic protection unavailable:', err.message); }
    }
  });
...
async function createTab(raw = null, activate = true, waitForNavigation = false) {
  ...
  // Fingerprint setup is intentionally fail-soft and never gates tab visibility.
  installFingerprintDefenses(tab).then(() => emitState()).catch(() => {});
...
function wireIpc() {
  ...
  ipcMain.handle('security-suite:run', (event) => assertUiSender(event) ? runSecuritySuite() : { testedAt: new Date().toISOString(), checks: [], summary: { pass: 0, fail: 0, 'not-tested': 0, total: 0 }, error: 'IPC sender denied' });
  ...
  ipcMain.on('settings:profile', (event, level) => {
    ...
    relayout();
    emitState();
    toast(`${level[0].toUpperCase() + level.slice(1)} privacy profile applied. Open a new tab for all fingerprint changes.`, 'success');
  });
