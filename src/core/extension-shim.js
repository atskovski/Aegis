...
const webRequestPattern=(url,pattern)=>{const p=String(pattern||'');if(p==='<all_urls>')return /^(?:https?|wss?):\/\//i.test(String(url||''));const bs=String.fromCharCode(92),special='\\.^$+?()[]{}|';const escaped=p.split('*').map((part)=>[...part].map((ch)=>special.includes(ch)?bs+ch:ch).join('')).join('.*');try{return new RegExp('^'+escaped+'$','i').test(String(url||''))}catch{return false}};
