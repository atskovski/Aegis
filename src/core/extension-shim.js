...
const area=(n)=>({get:(k)=>call('storage.'+n+'.get',k),getKeys:()=>call('storage.'+n+'.getKeys'),getBytesInUse:(k)=>call('storage.'+n+'.getBytesInUse',k),set:(v)=>call('storage.'+n+'.set',v),remove:(k)=>call('storage.'+n+'.remove',k),clear:()=>call('storage.'+n+'.clear')});
const ports=new Map();
