...
const webRequestPattern=(url,pattern)=>{const p=String(pattern||'');if(p==='<all_urls>')return /^(?:https?|wss?):\/\//i.test(String(url||''));const escaped=p.split('*').map((part)=>part.replace(/[.+?^$()|[\]\\]/g,'\\const webRequest={OnBeforeRequestOptions:Object.freeze({BLOCKING:'blocking',REQUEST_BODY:'requestBody'}),OnBeforeSendHeadersOptions:Object.freeze({REQUEST_HEADERS:'requestHeaders',BLOCKING:'blocking',EXTRA_HEADERS:'extraHeaders'}),OnHeadersReceivedOptions:Object.freeze({RESPONSE_HEADERS:'responseHeaders',BLOCKING:'blocking',EXTRA_HEADERS:'extraHeaders'}),onBeforeRequest:event('webRequest.onBeforeRequest'),onBeforeSendHeaders:event('webRequest.onBeforeSendHeaders'),onSendHeaders:event('webRequest.onSendHeaders'),onHeadersReceived:event('webRequest.onHeadersReceived'),onResponseStarted:event('webRequest.onResponseStarted'),onCompleted:event('webRequest.onCompleted'),onErrorOccurred:event('webRequest.onErrorOccurred'),handlerBehaviorChanged:(...a)=>call('webRequest.handlerBehaviorChanged',...a)};')).join('.*');try{return new RegExp('^'+escaped+'
...
privacy:{network:{webRTCIPHandlingPolicy:privacySetting('network','webRTCIPHandlingPolicy'),networkPredictionEnabled:privacySetting('network','networkPredictionEnabled')},services:{passwordSavingEnabled:privacySetting('services','passwordSavingEnabled'),autofillAddressEnabled:privacySetting('services','autofillAddressEnabled'),autofillCreditCardEnabled:privacySetting('services','autofillCreditCardEnabled')},websites:{thirdPartyCookiesAllowed:privacySetting('websites','thirdPartyCookiesAllowed'),hyperlinkAuditingEnabled:privacySetting('websites','hyperlinkAuditingEnabled'),referrersEnabled:privacySetting('websites','referrersEnabled'),protectedContentEnabled:privacySetting('websites','protectedContentEnabled')}},
...
${messageHook}
B.onBlockingRequest?.(async(p)=>{const type=String(p?.type||''),details=p?.details||{},merged={};for(const fn of [...listeners(type)]){const meta=webRequestMeta.get(fn);if(!meta||!meta.extra.includes('blocking')||!webRequestMatches(meta.filter,details))continue;try{let value=fn(details);if(value&&typeof value.then==='function')value=await value;if(!value||typeof value!=='object')continue;if(value.cancel===true)merged.cancel=true;const redirect=String(value.redirectUrl||value.redirectURL||'');if(redirect&&!merged.redirectUrl)merged.redirectUrl=redirect;if(Array.isArray(value.requestHeaders))merged.requestHeaders=value.requestHeaders;if(Array.isArray(value.responseHeaders))merged.responseHeaders=value.responseHeaders}catch{}}B.respondBlockingRequest?.(p?.requestId,merged)});
B.onEvent?.((p)=>{const type=String(p?.type||''),args=Array.isArray(p?.args)?p.args:[];if(type==='runtime.portMessage'){const port=ports.get(String(args[0]||''));if(port)port.onMessage._emit(args[1],port);return}if(type==='runtime.portDisconnect'){const id=String(args[0]||''),port=ports.get(id);if(port){ports.delete(id);port.onDisconnect._emit(port)}return}if(type==='runtime.onConnect'&&args[0]?.__aegisPort){const d=args[0],port=makePort(String(d.portId||''),d.name||'',d.sender||{});for(const fn of [...listeners('runtime.onConnect')])try{fn(port)}catch{}return}const a=listeners(type);for(const fn of [...a]){const meta=webRequestMeta.get(fn);if(type.startsWith('webRequest.')&&meta?.extra?.includes('blocking'))continue;try{fn(...args)}catch{}}});
})();`;
}

module.exports = { safeRel, localeMessages, commandList, webExtensionBootstrap };
,'i').test(String(url||''))}catch{return false}};
const webRequestMatches=(filter,details)=>{filter=filter||{};const urls=Array.isArray(filter.urls)?filter.urls:[];if(urls.length&&!urls.some((p)=>webRequestPattern(details?.url,p)))return false;const types=Array.isArray(filter.types)?filter.types:[];if(types.length&&!types.includes(String(details?.type||'')))return false;const tabId=Number(filter.tabId);if(Number.isFinite(tabId)&&tabId!==Number(details?.tabId))return false;return true};
const webRequestEvent=(name)=>({addListener:(fn,filter={},extraInfoSpec=[])=>{if(typeof fn!=='function')return;const a=listeners(name);if(!a.includes(fn))a.push(fn);webRequestMeta.set(fn,{name,filter:filter||{},extra:Array.isArray(extraInfoSpec)?extraInfoSpec.map(String):[]})},removeListener:(fn)=>{const a=listeners(name),i=a.indexOf(fn);if(i>=0)a.splice(i,1);webRequestMeta.delete(fn)},hasListener:(fn)=>listeners(name).includes(fn),hasListeners:()=>listeners(name).length>0});
const webRequest={OnBeforeRequestOptions:Object.freeze({BLOCKING:'blocking',REQUEST_BODY:'requestBody'}),OnBeforeSendHeadersOptions:Object.freeze({REQUEST_HEADERS:'requestHeaders',BLOCKING:'blocking',EXTRA_HEADERS:'extraHeaders'}),OnHeadersReceivedOptions:Object.freeze({RESPONSE_HEADERS:'responseHeaders',BLOCKING:'blocking',EXTRA_HEADERS:'extraHeaders'}),onBeforeRequest:webRequestEvent('webRequest.onBeforeRequest'),onBeforeSendHeaders:webRequestEvent('webRequest.onBeforeSendHeaders'),onSendHeaders:webRequestEvent('webRequest.onSendHeaders'),onHeadersReceived:webRequestEvent('webRequest.onHeadersReceived'),onResponseStarted:webRequestEvent('webRequest.onResponseStarted'),onCompleted:webRequestEvent('webRequest.onCompleted'),onErrorOccurred:webRequestEvent('webRequest.onErrorOccurred'),handlerBehaviorChanged:(...a)=>call('webRequest.handlerBehaviorChanged',...a)};
const api={runtime,extension:{getURL:runtime.getURL},storage:{local:area('local'),sync:area('sync'),session:area('session'),managed:area('managed'),onChanged:event('storage.onChanged')},tabs,windows,cookies,permissions:permissionsApi,
i18n:{getUILanguage:()=> 'en-US',getMessage:(key,subs)=>{let t=String(MSG[String(key||'')]||'');const a=Array.isArray(subs)?subs:[subs];a.filter(x=>x!==undefined).forEach((v,i)=>{t=t.split('$'+(i+1)).join(String(v))});return t}},
alarms:{create:(...a)=>call('alarms.create',...a),get:(...a)=>call('alarms.get',...a),getAll:(...a)=>call('alarms.getAll',...a),clear:(...a)=>call('alarms.clear',...a),clearAll:(...a)=>call('alarms.clearAll',...a),onAlarm:event('alarms.onAlarm')},
commands:{getAll:(...a)=>call('commands.getAll',...a),onCommand:event('commands.onCommand')},
