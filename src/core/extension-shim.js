...
const webRequestPattern=(url,pattern)=>{const p=String(pattern||'');if(p==='<all_urls>')return /^(?:https?|wss?):\/\//i.test(String(url||''));const escaped=p.split('*').map((part)=>part.replace(/[.+?^$()|[\]\\]/g,'\\$&')).join('.*');try{return new RegExp('^'+escaped+'$','i').test(String(url||''))}catch{return false}};
