...
const ENGINE_CAPABILITIES=Object.freeze([
 'views','sessions','protocols','networkInterception','permissions','proxy','downloads','devtoolsProtocol','isolatedWorlds','certificatePolicy'
]);
function assertEngineAdapter(engine){
 if(!engine||typeof engine!=='object')throw new TypeError('Browser engine adapter is required.');
 for(const name of ['name','version','capabilities','createSession','createView','attachView','detachView','destroyView','registerProtocol','fetch','getRuntimeVersions']){
  if(typeof engine[name]!=='function')throw new TypeError('Engine adapter missing '+name+'().');
 }
...
module.exports={ENGINE_CAPABILITIES,assertEngineAdapter,engineEvidence};
