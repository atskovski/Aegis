...
function configurePrivacySession({ ses, tab, getSettings, chromiumVersion, onStats, onPermissionBlocked, onPermissionPrompt, onSensitiveAccess, onNetworkAccess, trackerLearner, getFilterRules, isTemporarilyAllowed }) {
  ...
  ses.webRequest.onBeforeRequest({ urls: ['*://*/*'] }, (details, callback) => {
    ...
    const shouldBlock = tab.shieldsEnabled && !tab.compatibilityMode && thirdParty && (highConfidence || fpScript || learned);
    if (shouldBlock) {
      ...
      noteBlocked(tab, details.url); if (typeof onNetworkAccess === 'function') onNetworkAccess({ url: details.url, blocked: true, category: category || (fpScript ? 'fingerprint' : (learned ? 'heuristic' : 'tracker')), resourceType: details.resourceType }); onStats(tab); return callback({ cancel: true });
    }
