...
class AegisExtensionRuntime{
  ...
  handleBlockingWebRequestResponse(sender,payload={}){
    const requestId=String(payload.requestId||''),pending=this.pendingBlockingRequests.get(requestId);
    if(!pending||pending.host!==sender||pending.extensionId!==String(payload.extensionId||''))return false;
    this.pendingBlockingRequests.delete(requestId);
    const raw=payload.response&&typeof payload.response==='object'?payload.response:null;if(!raw){pending.resolve(null);return true}
    const safe={cancel:raw.cancel===true},redirect=String(raw.redirectUrl||raw.redirectURL||'');
    if(/^https?:\/\//i.test(redirect))safe.redirectURL=redirect;
    else if(/^aegis-extension:\/\//i.test(redirect)){
      try{
        const e=this.items.get(pending.extensionId),u=new URL(redirect),rel=decodeURIComponent(u.pathname.replace(/^\//,''));
        if(e&&u.hostname===e.resourceToken&&webAccessibleResourceAllowed(e,rel)&&this.extensionFile(e,rel))safe.redirectURL=redirect;
      }catch{}
    }
    if(Array.isArray(raw.requestHeaders))safe.requestHeaders=raw.requestHeaders.slice(0,256);
    if(Array.isArray(raw.responseHeaders))safe.responseHeaders=raw.responseHeaders.slice(0,256);
    pending.resolve(safe);return true;
  }
  privacyValue(key){
