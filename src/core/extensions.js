...
function webExtensionBootstrap(ext, bridgeGlobal='__aegisExtensionBridge'){
  const id=JSON.stringify(ext.id), token=JSON.stringify(ext.resourceToken), manifest=JSON.stringify(ext.manifest), messages=JSON.stringify(localeMessages(ext)), bridge=JSON.stringify(bridgeGlobal);
  return `(()=>{'use strict';
const ID=${id},TOKEN=${token},M=Object.freeze(${manifest}),MSG=Object.freeze(${messages}),B=globalThis[${bridge}];if(!B)return;
const events=new Map(),listeners=(name)=>{if(!events.has(name))events.set(name,[]);return events.get(name)},event=(name)=>({addListener:(f)=>{if(typeof f==='function'&&!listeners(name).includes(f))listeners(name).push(f)},removeListener:(f)=>{const a=listeners(name),i=a.indexOf(f);if(i>=0)a.splice(i,1)},hasListener:(f)=>listeners(name).includes(f),hasListeners:()=>listeners(name).length>0});
const call=(m,...a)=>B.call(m,a),area=(n)=>({get:(k)=>call('storage.'+n+'.get',k),set:(v)=>call('storage.'+n+'.set',v),remove:(k)=>call('storage.'+n+'.remove',k),clear:()=>call('storage.'+n+'.clear')});
const runtime={id:ID,getManifest:()=>M,getURL:(p='')=>'aegis-extension://ext/'+TOKEN+'/'+String(p).replace(/^\\/+/,''),getPlatformInfo:()=>call('runtime.getPlatformInfo'),getBrowserInfo:()=>call('runtime.getBrowserInfo'),openOptionsPage:()=>call('runtime.openOptionsPage'),reload:()=>call('runtime.reload'),sendMessage:(...a)=>call('runtime.sendMessage',...a),onMessage:event('runtime.onMessage'),onInstalled:event('runtime.onInstalled'),onStartup:event('runtime.onStartup')};
...
const api={runtime,extension:{getURL:runtime.getURL},storage:{local:area('local'),session:area('session'),onChanged:event('storage.onChanged')},tabs,permissions:permissionsApi,
i18n:{getUILanguage:()=> 'en-US',getMessage:(key,subs)=>{let t=String(MSG[String(key||'')]||'');const a=Array.isArray(subs)?subs:[subs];a.filter(x=>x!==undefined).forEach((v,i)=>{t=t.replace(new RegExp('\\\\$'+(i+1),'g'),String(v))});return t}},
