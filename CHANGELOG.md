...
- Added a formal PRD and brand system for Aegis / Sentinel / Security Suite.
- Added safe generic cosmetic ad filtering in Strict mode and broader cosmetic filtering in Maximum mode.
- Added Maximum-mode reduction for local font access and selected device/browser APIs.
- Added stronger Maximum-mode WebGL readPixels perturbation and canvas text-metric quantization.
- Increased UI type sizes across privacy panels, settings, diagnostics, evidence rows, and permission dialogs for legibility.
- Added committed benchmark evidence and a security-validation summary without publishing screenshots that expose public IP/ISP/location data.
- Added regression tests for cosmetic filtering behavior.
- Preserved compatibility by not globally disabling core JavaScript primitives merely to improve synthetic benchmark scores.
