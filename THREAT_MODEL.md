...
The first tab is attached before remote navigation, and Aegis waits for the new-document privacy/fingerprint preload to install before the first public page is requested. That gate is bounded so an unavailable Chromium debugger path cannot leave the browser permanently unusable; failures remain fail-soft and are surfaced by diagnostics. Fresh System sessions do not race a proxy mutation against initial navigation.
...
SponsorBlock requires contacting its service when enabled. Decentraleyes-style local replacement libraries are not bundled in v0.9; Aegis instead isolates common public-CDN cookies/referrers and blocks high-confidence tracking endpoints. This should not be described as full local-library replacement parity.

## v0.9 additions to the threat surface

Guardian explicitly models advertising APIs, local-font enumeration, hardware-device APIs, background beacon/ping tracking, canvas/WebGL fingerprinting, WebRTC host candidates, cosmetic ad injection, and route/public-IP visibility. The mitigations reduce these surfaces but do not make a Chromium browser anonymous or indistinguishable from every other browser.
